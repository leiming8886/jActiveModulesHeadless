...
public class ParamStatistics implements Serializable{
   ...
   */
  public void calculateMeanAndStd(Node [] nodes,int trials, int maxThreads){
    ...
    for(int i=0;i<maxThreads;i++){
      Thread thread = new MeanAndStdThread(overall_tallyx,overall_tallyx_2,trials,nodes,rand);
      //thread.start();
      threads.add(thread);
    }
...
class MeanAndStdThread extends Thread{
  ...
  private Node[] nodes;
  private Random rand;
  public MeanAndStdThread(double [] overall_tallyx,double [] overall_tallyx_2,int total_trials,Node [] nodes,Random rand){
