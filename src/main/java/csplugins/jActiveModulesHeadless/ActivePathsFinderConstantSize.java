...
*/
public class ActivePathsFinderConstantSize {
	 ...
	 * 
	 * @param attrNames
	 *            The names of the attributes which correspond to significance
	 * @param Network
	 ...
	 */
	 public ActivePathsFinderConstantSize(HashMap expressionMap, int attrNamesLength,
	 ...
	 */
	private void runGreedySearchConstantSize() {
		
		runGreedySearchConstantSize(network.getNodeList(), apfParams.getSizeSubnets());
		
	}

	private void runGreedySearchConstantSize(Collection seedList, int sizeSubnet) {
     ...
     //  while(test){	  
			...
			//TODO:initiate component of size sizeComponent
			nodeTemp = (Node) nodeIterator.next();
			node2BestComponent.put(nodeTemp, component);
			GreedySearchThreadConstantSize gst = new GreedySearchThreadConstantSize(network,
					apfParams,nodeTemp,
					component, nodes);
