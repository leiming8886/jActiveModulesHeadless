...
//		}
		...
		/*final Network overview = ServicesUtil.NetworkFactoryServiceRef.createNetwork();
		overview.getRow(overview).set("name", "jActiveModules Search Result "+ runCount++);
		
		Set<Node>  path_nodes = new HashSet<Node>();
		for (int i=0; i< subnetworks.length; i++){
			Node newNode = overview.addNode(); //Cytoscape.getNode(subnetworks[i].getTitle(), true);
		...
		ServicesUtil.NetworkManagerServiceRef.addNetwork(overview);
		
		//Edges indicate that nodes in nested networks exist in both nested networks
		Set<Edge>  path_edges = getPathEdges(overview, path_nodes); //new HashSet<Edge>();
		//final Network overview = Cytoscape.createNetwork(path_nodes, path_edges, "jActiveModules Search Result "+ runCount++, Network, false);
		...
		CyTable EdgeAttrs = this.Network.getDefaultEdgeTable(); //Cytoscape.getEdgeAttributes();
		Iterator it = path_edges.iterator();
		while(it.hasNext()){
			Edge aEdge = (Edge) it.next();
			int NumberOfSharedNodes = getNumberOfSharedNodes((Network)aEdge.getSource().getNetworkPointer(), 
					(Network)aEdge.getTarget().getNetworkPointer());
			
			int minNodeCount = Math.min(aEdge.getSource().getNetworkPointer().getNodeCount(), 
								aEdge.getTarget().getNetworkPointer().getNodeCount());

			//EdgeAttrs.setAttribute(aEdge.getIdentifier(), "jActiveModules_nodeCount_min_two", minNodeCount);
			overview.getRow(aEdge).set("jActiveModules_nodeCount_min_two", minNodeCount);
			
			//EdgeAttrs.setAttribute(aEdge.getIdentifier(), "jActiveModules_nodeOverlapCount", NumberOfSharedNodes);
			...
			//EdgeAttrs.setAttribute(aEdge.getIdentifier(), EDGE_SCORE, overlapScore);
